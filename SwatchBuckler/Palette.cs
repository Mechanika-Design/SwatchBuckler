...
namespace SwatchBuckler
{
    ...
    public abstract class Palette : IPalette
    {
        ...
        private static readonly List<IPalette> _serializerCache;
        ...
        /// <summary>
        /// Initializes static members of the <see cref="Palette"/> class.
        /// </summary>
        static Palette()
        {
            _serializerCache = new List<IPalette>();
        }
        ...
        public static IEnumerable<IPalette> Alls
        {
        ...
        public static IPalette GetSerializer(string fileName)
        {
            IPalette result;
            ...
            foreach (IPalette check in Alls)
            {
                using (FileStream file = File.OpenRead(fileName))
                {
                    if (check.CanReadFrom(file))
                    {
                        result = check;
                        break;
                    }
        ...
        private static void CreateFilters()
        {
            ...
            foreach (IPalette serializer in _serializerCache.Where(serializer => !(string.IsNullOrEmpty(serializer.DefaultExtension) || openExtensions.Contains(serializer.DefaultExtension))))
            {
        ...
        private static void Loads()
        {
            ...
            foreach (Type type in AppDomain.CurrentDomain.GetAssemblies().SelectMany(assembly => GetLoadableTypes(assembly).Where(type => !type.IsAbstract && type.IsPublic && typeof(IPalette).IsAssignableFrom(type))))
            {
                try
                {
                    _serializerCache.Add((IPalette)Activator.CreateInstance(type));
                }
        ...
        /// <param name="stream">The <see cref="Stream" /> used to write the palette.</param>
        /// <param name="palette">The <see cref="ColorCollection" /> to serialize.</param>
        void IPalette.Serialize(Stream stream, ColorCollection palette)
        {
            this.Serialize(stream, palette);
        }
        ...
        /// <param name="stream">The <see cref="Stream" /> that contains the palette to deserialize.</param>
        /// <returns>The <see cref="ColorCollection" /> being deserialized.</returns>
        ColorCollection IPalette.Deserialize(Stream stream)
        {
            return this.Deserialize(stream);
        }
        ...
        /// </summary>
        /// <value>The default extension for files generated with this palette format.</value>
        string IPalette.DefaultExtension
        {
        ...
        /// <param name="stream">The stream.</param>
        /// <returns><c>true</c> if this instance can read palette data from the specified stream; otherwise, <c>false</c>.</returns>
        bool IPalette.CanReadFrom(Stream stream)
        {
